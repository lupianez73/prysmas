...
gulp.task('start:server', () => {
    ...
    config = require(`./${serverPath}/config/environment`).default || require(`./${serverPath}/config/environment`);
...
gulp.task('start:server:prod', () => {
    ...
    config = require(`./${paths.dist}/${serverPath}/config/environment`).default || require(`./${paths.dist}/${serverPath}/config/environment`);
...
gulp.task('serve', gulp.series(
    gulp.parallel('clean:tmp', 'lint:scripts', 'inject', 'copy:fonts:dev', 'env:all'),
    // 'webpack:dev',
...
gulp.task('test:server:coverage', gulp.series(
    'coverage:pre',
    'env:all',
    'env:test',
    'coverage:unit',
...
gulp.task('test:e2e', gulp.series(
    gulp.parallel('webpack:e2e', 'env:all', 'env:test', 'start:server', 'webdriver_update'),
    cb => {
...
gulp.task('build', gulp.series(
    ...
    gulp.parallel('copy:extras', 'copy:assets', 'copy:fonts:dist', 'copy:server', 'webpack:dist'),
    'revReplaceWebpack'
));
